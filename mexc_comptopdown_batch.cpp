 ...
 * ===============================================================================
 * Compile: mex -g -I./ mexc_comptopdown_batch.cpp comptopdown.cpp util.cpp filter.cpp checkparts.cpp
 * parallel
 * mex -v CXXFLAGS="\$CXXFLAGS -Wall -std=c++0x -fopenmp" LDFLAGS="\$LDFLAGS -fopenmp -lmpi" COPTIMFLAGS='-O3 -DNDEBUG' -I./ mexc_comptopdown_batch.cpp comptopdown.cpp util.cpp filter.cpp checkparts.cpp
 * Runing Example: Output = mexc_comptopdown_batch(data,layers, 2, 5, 4, 0.9, [2 1 1 1 0.5 0.2 0.1], rotation);
 * ===============================================================================
 *
 * comptoptopdown(int *morients, int imh, int imw, int layers, int numlayer,int numOrient,int numsub,
 * double discount,double *param, double rotation)
 */
int totLayers = 7;
unsigned char **morients;
                 ...
                 int nrhs, const mxArray *prhs[])                
{
    ...
    if((nlhs!=1)||(nrhs!=8))
    {
        mexErrMsgTxt("1 output and 8 input needed!\n");
        return;
    }
    ...
    // image size
    int *imh = (int*)malloc(sizeof(int)*numdata);
    int *imw = (int*)malloc(sizeof(int)*numdata);
    morients = (unsigned char **)malloc(numdata*sizeof(unsigned char *));
    for(i=0; i< numdata; i++)
    {
        // pcell = (const mxArray *)mxGetPr(mxGetField(lambda, kstruct, "elements"));
        pcell = (const mxArray*)mxGetCell(prhs[0], i);
        if(pcell==NULL){
               morients[i]=NULL;
               imh[i] = 0;
               imw[i] = 0;
        ...
        if(mxGetClassID(pcell)!=mxUINT8_CLASS)
        {
            mexErrMsgTxt("Error: The morients data is not unsigned char* or uint8!\n");
            return;
        }    
        imh[i] = dims[0];
        imw[i] = dims[1];
        /*allocate space for it*/
        morients[i] = (unsigned char*)mxGetPr(pcell);
    ...
    */
    ...
    /*in put variable 8: number of orientations*/
    rotation  = (int)mxGetScalar(prhs[7]);  
    
    // only 8 parameters in this one
    ...
    if(numlayer>NUM_OF_LAYER){
        mexPrintf("the total layer is : %d", NUM_OF_LAYER);
     ...
     *==============================================
     */
    // if(flag) numsub = numsub+1;
    // printf("the numsub with changed: %d\n", numsub);
    struct Data *output = (Data*)malloc(sizeof(Data)*numdata);
    //#pragma omp parallel for 
    for(i=0; i< numdata; i++)
    {
        if(morients[i]!=NULL)
            //comptopdown_full(&output[i], morients[i], imh[i], imw[i], model, numlayer, numsub, numOrient, discount, param, rotation);
            comptopdown_fast(&output[i], morients[i],imh[i], imw[i], model, numlayer, numsub, numOrient, discount, param, rotation);
            //comptopdown(&output[i], morients[i],imh[i], imw[i], model, numlayer, numsub, numOrient, discount, param, rotation);
    }
    /* =============================================
    ...
    for(jstruct=0; jstruct<numdata; jstruct++)
    {
        ...
        {   
            ...
            for(k=0; k< output[jstruct].lambda[j].numel; k++) //
            {
                ...
                dimsOutput[1] = 4;//pid, numlayer, py,px
...
//     }
    ...
    //free space from data & parameters  
    free(morients);
    free(imh); free(imw);
