 ...
 * ===============================================================================
 * Compile with Debug: mex -g -I./ mexc_complearn.cpp complearn.cpp util.cpp filter.cpp checkparts.cpp
 * OR: mex -v CFLAGS="\$CFLAGS -Wall -std=c++0x" -g -I./ mexc_complearn.cpp checkparts.cpp complearn.cpp compinference.cpp util.cpp filter.cpp
 * Runtime: mex -v COPTIMFLAGS='-O3 -DNDEBUG -fopenmp' -I./ mexc_complearn.cpp complearn.cpp util.cpp filter.cpp checkparts.cpp compinference.cpp
 * mex -v CXXFLAGS="\$CXXFLAGS -Wall -std=c++0x --openmp" COPTIMFLAGS='-O3 -DNDEBUG' -I./ mexc_complearn.cpp complearn.cpp util.cpp filter.cpp checkparts.cpp compinference.cpp
 * 
 * 
 * mex -v CXXFLAGS="\$CXXFLAGS -Wall -std=c++0x" LDFLAGS="\$LDFLAGS -fopenmp" COPTIMFLAGS='-O3 -DNDEBUG' -I./ mexc_complearn.cpp complearn.cpp util.cpp filter.cpp checkparts.cpp compinference.cpp
 *
 * compile parallel with: mex -v CXXFLAGS="\$CXXFLAGS -Wall -std=c++0x -fopenmp" LDFLAGS="\$LDFLAGS -fopenmp -lmpi" COPTIMFLAGS='-O3 -DNDEBUG' -I./ mexc_complearn.cpp complearn.cpp util.cpp filter.cpp checkparts.cpp compinference.cpp
 * Runing Example: model = mexc_complearn(data,layers, 2, 2,4,10, 0.9, [2 1 1 1 0.5 0.2 0.1], 0.9);
 * ===============================================================================
 *
                 ...
                 int nrhs, const mxArray *prhs[])                
{
    ...
    //flag to control the format of ptypes
    bool flag=false;
    
    //PROGRAM
    if((nlhs!=1)||(nrhs!=9))
    {
        mexErrMsgTxt("One output and 9 input needed");
        return;
    }
    ...
    */
    ...
    Parts* model = (Parts*)malloc((numlayer+1)*sizeof(struct Parts));
    layers = prhs[1];
    ...
    //for(ifield = 0; ifield<NUM_FIELD_LAYERS; ifield++)
        for(jstruct=0; jstruct<numlayer; jstruct++)
        {
            ...
            else if(ptypes!=NULL)
            {
               ...
               if((mxGetNumberOfElements(ptypes)>numsub)&&model[jstruct].ptypes_[numsub].ntypes>0 && jstruct==numlayer-1)
                   flag = true;
            }
            else
            {
            ...
            if(ntypes !=NULL)
            {
                ...
                for(i=0;i<dims[1]*dims[0];i++)
                    model[jstruct].ntypes_[i] = (int)ntypes[i]; //printf("%d\n", model[jstruct].ntypes_[i]);
            ...
            else model[jstruct].radius = 0.5*radius;
    ...
    learningcomposition(examples, numdata, model, numlayer,numsub, numOrient, radius, percent, param, discount, flag);
    ...
    for(jstruct=0; jstruct<numlayer+1; jstruct++)
    {
        ...
        mxArray *partstruct = mxCreateStructMatrix(numsub+1, 1, 2, (const char**)subfieldnames);//2 fields p and p
        ...
        if(jstruct<numlayer)
        {
            if(jstruct==0)
                ...
                if(flag)
                    numparts = model[jstruct].ptypes_[numsub].ntypes; 
                else
    ...
    for(jstruct=0; jstruct<numlayer+1; jstruct++)
    {
       ...
       */
       if(model[jstruct].ptypes_)
       {    
           free(model[jstruct].ptypes_);
           model[jstruct].ptypes_=NULL;
       }
       if(model[jstruct].ntypes_!=NULL)
       {
           free(model[jstruct].ntypes_);
           model[jstruct].ntypes_ = NULL;
       }
